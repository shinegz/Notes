...
​	每个HTML元素都属于一个层叠上下文。文档中的层叠上下文由满足以下任意一个条件的元素形成：
...
​	层叠水平（Stacking Level）决定了**同一个层叠上下文**中元素在**`z`轴**上的**显示顺序**。所有的元素都有层叠水平，包括层叠上下文元素（**层叠上下文元素可能处于另一层叠上下文**）。普通元素的层叠水平优先由层叠上下文决定，因此，层叠水平的比较只有在当前层叠上下文元素中才有意义。给定层叠上下文中的每个元素都具有一个层叠水平，具有更高层叠水平的元素盒子总是在具有较低层叠水平的盒子的上面。当元素的层叠水平一致时，在DOM流中处于后面的元素会覆盖前面的元素。

​	注意，层叠水平和css的`z-index`属性不是同一个概念，`z-index`可以影响层叠水平，但只限于定位元素以及Flex盒子的孩子元素。

#### 层叠顺序（order）

层叠顺序（stacking order），表示元素发生层叠时有着特定的垂直显示顺序。层叠顺序的规则如下：
...
- 浮动盒：非定位浮动元素。
- 行内盒：文档流中行内级别非定位子元素。
- `z-index: 0` ：定位元素（z-index：auto）和不支持z-index的层叠上下文元素（相当于z-index：auto级别）。
