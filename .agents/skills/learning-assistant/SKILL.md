...
description: Autonomous learning agent that studies materials on behalf of the user and outputs distilled knowledge in a comprehensive learning summary. **ALWAYS use this skill when the user wants to learn, understand, or get a summary of any topic or materials.** Trigger phrases include: "help me learn", "I want to understand", "study this", "summarize these", "research this topic", "teach me about", "explain this to me", "digest this documentation", "create a learning summary", "synthesize these materials", or when user provides multiple files/links/URLs asking for knowledge synthesis. Also triggers for Chinese phrases: "帮我学习", "我想了解", "研究一下", "学习总结", "帮我理解". The skill reads provided materials, fills knowledge gaps through research, and produces a standalone learning summary containing distilled knowledge—so the user learns by reading the summary, not by being directed to use the original materials.
---
...
11. **术语溯源 + 概念演变**：交代术语的提出时间、提出者、演变过程。帮助读者理解理论往往滞后于实践，避免产生"这个概念是新发明"的误解。
12. **术语冲突显式澄清**：当同一术语在不同语境有不同含义时，显式说明两种用法的区别和关系，避免读者陷入概念困境。
13. **核心结论精简**：核心结论控制在 3 条左右，每条一句话。过多的结论会稀释重点。
14. **类比择优**：若文章有多个类比，选择最能体现核心定义的类比放在心智模型位置，次要类比放在正文补充。
15. **来源可追溯**：关键论断要有来源引用，增加可信度并方便读者追溯原文。
...
### For article-style learning deliverables

When the user is not just asking to "learn a topic" but wants a **finished article / learning note / study summary document**, use a slightly more guided interaction pattern before you start writing.

**Use this pattern for requests like:**
- "学完这个并写一篇文章"
- "帮我整理成学习总结"
- "研究完后输出一篇完整文章"
- "不要只总结，写成一篇能直接看的内容"
...
**Interaction rules:**
1. **Clarify the deliverable before execution.** Confirm what the user actually wants produced: learning summary, article, note, practical guide, overview, or mixed style.
2. **Ask one key question at a time.** Do not dump a long questionnaire unless the task is genuinely blocked. Prefer the single highest-leverage question first, such as article style, target depth, or intended audience.
3. **Confirm structure before heavy writing.** Once you understand the goal, propose a concise outline or writing approach and wait for user confirmation before drafting the full article.
4. **Then execute the research-and-writing flow.** After structure is approved, study materials, fill knowledge gaps, and write the final document.
5. **Do one post-delivery confirmation round.** After delivering the article, ask whether the user wants a shorter version, a more practical version, better formatting, or further expansion.

**Recommended micro-flow:**
1. Confirm topic and output type
2. Ask one focused clarification question
3. Read / research materials
4. Present recommended structure or article approach
5. Wait for approval
6. Write the full document
7. Ask whether to refine further

**Preferred clarification priorities for article tasks:**
- First priority: article style or output mode
- Second priority: target depth
- Third priority: emphasis area or intended reader

If the user already gives all of this clearly, skip the questions and move straight to a proposed structure.

**Lightweight example:**
```text
我会先把资料学完，再整理成一篇成文的学习文章。
先确认一个点：你更希望这篇文章偏系统综述、偏实战方法，还是两者结合？
```

After the user answers:
```text
我建议按「概念框架 + 核心方法 + 案例 + 落地建议」来展开。
如果这个结构没问题，我就按这个方案开始整理并写成完整文章。
```
