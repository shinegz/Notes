...
写 `concepts/`、`syntheses/`、`comparisons/`、实质性修订 `overview.md` 时，先读 **`docs/ingest-structure-guide.md`** 确定页面类型和结构，再读 **`docs/ingest-style-guide.md`** 润色表达。Agent 执行前**必须强制检查**环境中是否已安装以下 skill。若缺失，**必须询问用户是否安装**，**未安装完成前不得执行成稿操作**：
