# AI Fundamentals 门类

AI 理论基础：历史脉络、重要论文、技术演进、LLM 原理。

## 来源导航

### Transformer
- [[ai-fundamentals/sources/attention-is-all-you-need|Attention Is All You Need]] — Transformer 架构原始论文
- [[ai-fundamentals/sources/the-illustrated-transformer|The Illustrated Transformer]] — Transformer 可视化教程
- [[ai-fundamentals/sources/transformer-taxonomy|Transformer Taxonomy]] — 22 个模型 + 11 个架构改进文献综述

### 预训练
- [[ai-fundamentals/sources/bert-pre-training|BERT]] — 双向 Transformer 预训练
- [[ai-fundamentals/sources/gpt3-language-models-few-shot|GPT-3]] — Few-Shot 学习

### Scaling Laws
- [[ai-fundamentals/sources/scaling-laws-kaplan|Scaling Laws]] — 模型、数据、算力的规模化规律
- [[ai-fundamentals/sources/chinchilla|Chinchilla]] — 计算最优 scaling：20:1 token-to-param 比例

### Alignment
- [[ai-fundamentals/sources/instructgpt|InstructGPT]] — RLHF 与人类反馈对齐
- [[ai-fundamentals/sources/rlhf-from-feedback|RLHF from Human Feedback]] — RLHF 奠基论文（summarization）
- [[ai-fundamentals/sources/dpo|DPO]] — 直接偏好优化，无需显式奖励模型

### 效率优化
- [[ai-fundamentals/sources/flash-attention|Flash Attention]] — IO感知的精确注意力
- [[ai-fundamentals/sources/lora|LoRA]] — 低秩适配，参数高效微调
- [[ai-fundamentals/sources/gqa|GQA]] — Grouped-Query Attention，推理内存优化
- [[ai-fundamentals/sources/rope|RoPE]] — 旋转位置编码

### Agent / Tool Use
- [[ai-fundamentals/sources/llm-powered-autonomous-agents|LLM Powered Autonomous Agents]] — Lilian Weng 的 Agent 系统综述
- [[ai-fundamentals/sources/react-chain-of-thought|ReAct]] — 推理与行动协同框架
- [[ai-fundamentals/sources/toolformer|Toolformer]] — 语言模型自学使用工具
- [[ai-fundamentals/sources/agentbench|AgentBench]] — LLM Agent 能力评测基准

### 历史与趋势
- [[ai-fundamentals/sources/history-of-artificial-intelligence|AI History (Wikipedia)]] — AI 发展史完整脉络
- [[ai-fundamentals/sources/ai-index-stanford-hai|AI Index]] — Stanford HAI 年度 AI 指数报告

### 经典工作
- [[ai-fundamentals/sources/alexnet|AlexNet]] — 2012 年 ImageNet 冠军，深度学习革命起点
- [[ai-fundamentals/sources/resnet|ResNet]] — 残差学习，极深网络训练
- [[ai-fundamentals/sources/word2vec|word2vec]] — 词嵌入与向量语义
- [[ai-fundamentals/sources/gan|GAN]] — 生成对抗网络

## 概念页

- [[ai-fundamentals/concepts/transformer|Transformer]]
- [[ai-fundamentals/concepts/attention-mechanism|Attention Mechanism]]
- [[ai-fundamentals/concepts/language-model-training|Language Model Training]]
- [[ai-fundamentals/concepts/alignment|Alignment]]
- [[ai-fundamentals/concepts/llm-agents|LLM Agents]]
- [[ai-fundamentals/concepts/tokenization|Tokenization]]
- [[ai-fundamentals/concepts/embedding|Embedding]]
- [[ai-fundamentals/concepts/scaling-laws|Scaling Laws]]
- [[ai-fundamentals/concepts/vector-database|Vector Database]]
- [[ai-fundamentals/concepts/memory|Memory]]
- [[ai-fundamentals/concepts/rag|RAG]]
- [[ai-fundamentals/concepts/chain-of-thought-react|Chain-of-Thought & ReAct]]
- [[ai-fundamentals/concepts/fine-tuning|Fine-tuning]]
- [[ai-fundamentals/concepts/function-calling|Function Calling]]

## 参见

- [[index]] — 总索引
- [[ai-fundamentals/overview]] — AI Fundamentals 总览

## 综合文

- [[ai-fundamentals/syntheses/ai-core-concepts|AI核心概念体系]] — 基于 source 资料的概念地图（Token、Context、Prompt、LLM、Tool、Agent、MCP、Skill）
- [[ai-fundamentals/syntheses/token-lifecycle|一个 Token 的一生]] — 用图文故事解释 LLM 运行原理
