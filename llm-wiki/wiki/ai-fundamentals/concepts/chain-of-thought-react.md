...
**CoT = 让 LLM 显式写出推理步骤；ReAct = CoT + 行动，让 LLM 边思考边调用工具[[ai-fundamentals/sources/react-chain-of-thought|1]]。**
...
[[ai-fundamentals/sources/gpt3-language-models-few-shot|GPT-3]] 的 few-shot prompting 证明，给模型几个示例就能引导它完成新任务[[ai-fundamentals/sources/gpt3-language-models-few-shot|2]]。CoT 更进一步：**示例中不仅给答案，还给推理过程**。
...
[[ai-fundamentals/sources/react-chain-of-thought|1]] 提出：将 **推理痕迹（Thought）** 和 **任务行动（Action）** 交织执行，形成一个循环。
...
[[ai-fundamentals/sources/react-chain-of-thought|1]] 实验证明：
- 优于单独的 CoT（只推理不行动）
- 优于单独的 Action（只行动不推理）
- 推理痕迹提高了可解释性和可信度
...
- [[ai-fundamentals/sources/react-chain-of-thought|ReAct]] — 推理与行动协同框架原始论文
- [[ai-fundamentals/sources/llm-powered-autonomous-agents|LLM Powered Autonomous Agents]] — CoT 变体综述（CoT-SC, ToT, Reflexion 等）
