...
这个公式来自 Anthropic 官方定义，已在行业内形成共识。关键洞察：评估一个 Agent 的效果时，评估的是 **model + harness 的组合**，而非模型单独的能力。
...
**Harness 不是：**
- 更长的 system prompt
- 某个框架（LangChain、CrewAI）
- 可选的「锦上添花」

**Harness 是：** 让模型能够作为 Agent 行动起来的 **外循环系统**——包含计划分解、持久状态、工具编排、验证门控、反馈回路、回退机制、人机交接点和审计日志。

---

## 为什么需要 Harness？—— 五大根本挑战

| 挑战 | 本质 | 为什么模型解决不了 |
|------|------|-------------------|
| **状态持久性** | Agent 需跨时间、跨 session 记住做过什么 | 模型无状态，context window 有上限 |
| **目标一致性** | 长任务中容易漂移、自嗨、提前宣布完成 | 模型缺少外部锚点校准「什么才算真正完成」 |
| **行动可验证性** | 每一步都是概率性的，需区分「做了」和「做对了」 | 模型评价自己结果时存在自我表扬倾向 |
| **熵增抑制** | 持续产出累积冗余、漂移和不一致 | 模型会复制已有模式，哪怕是坏模式 |
| **人机边界** | 何时自主、何时交给人 | 模型没有可靠的「不确定性自觉」 |

这些挑战的共性：**不能靠更聪明的模型单独解决**。它们是系统性问题，需要工程化的外循环来约束。

---

## 三层抽象：Prompt → Context → Harness
...
Context Engineering 是 Harness Engineering 的子集，但 Harness 还包含：
- 多步结构
- 工具中介
- 验证门
- Durable state

这些都超出了单步 context 的范畴。
...
**问题：** 长时 Agent 的核心痛点，就像项目组里工程师每次换班都完全失忆。
...
session 1 (initializer):
  ├─ init.sh          # 启动脚本
  ├─ progress.txt     # 进度日志
  ├─ feature-list.json # 200+ 条功能，初始全 failing
  └─ git commit       # 基线快照
...
**Context Anxiety：** Anthropic 发现即使有 compaction，Agent 仍因「上下文太满」而行为退化。解决方案是 **context reset**——直接给下一个 Agent 全新 context，通过外化状态工件传递信息。

### 2. Decomposition & Plans（分解与计划）

**问题：** 给 Agent 说「build a clone of claude.ai」，它会一把梭——结果要么上下文爆了，要么做到一半宣布「完成」。
...
**设计原理：** 计划必须是 **一等工件**——写入文件系统、被版本管理、被后续 Agent 可读取、被验证门引用。存在于对话里的计划，只是一次想法。

### 3. Feedback Loops（反馈回路）

**Guides vs Sensors 2×2 矩阵：**
...
**Anthropic 发现：** Evaluator 不是永远必要——当底模能力跨过阈值后，evaluator 从「必要部件」退化为「额外开销」。好的 harness 是 **与模型能力边界共同演进的可裁剪系统**。

### 4. Legibility（可感知性）

**OpenAI 判断：** 凡是不在 Agent 运行时可见范围内的知识，就等于不存在。
...
**警告：** 超长 `AGENTS.md` 会快速腐烂、挤占上下文、让所有约束同时失焦。更好的做法是把它变成 **目录索引**，真正知识拆散到结构化文档。

### 5. Tool Mediation（工具中介）

**问题：** MCP 生态爆发后，一个 Agent 可能连接几十个 MCP 服务器、上百个工具——token 成本暴增、延迟上升、Agent 迷失方向。

**Anthropic 核心思路：** 不要让模型直接调用工具，让模型 **写代码来调用工具**。
...
本质：**把工具使用从模型的内循环，挪到更高效的外部执行回路**。

### 6. Entropy Control（熵增抑制）

**问题：** 全自动 Agent 代码库会不断复制既有模式——哪怕那些模式不均匀、次优甚至糟糕。

**OpenAI 做法：**
- 最初：人每周花 ~20% 时间清理「AI slop」
- 后来：系统化——Documentation consistency agents、Refactor agents、Architectural enforcement (CI)

**设计原理：** Harness 不只负责「让 Agent 跑起来」，还负责 **持续抑制 Agent 放大的系统噪声**。这是它与「Agent 框架」的本质区别——框架关心启动和编排，harness 关心长期可治性。

---

## Harnessability（可 Harness 程度）

不是每个系统都同样容易被 harness。
...
**判断：** 在 Agent 时代，团队工程基础设施质量（CI 完善度、文档结构化程度、架构边界清晰度）不再只是「工程素养」问题——它直接决定了 Agent 能在你的系统上走多远。
...
**定位：** Harness 是 AgentOS 的 **用户态层**。AgentOS 是内核——管调度、管隔离、管资源。Harness 是用户态的 shell 和 daemon——管任务分解、管状态续航、管验证反馈、管人机交接。

---

## 五个典型症状

如果观察到这些，说明你的「Agent 系统」还停在临时搭伙阶段：
